...
function M.pick(prompt, src, onclose, opts)
  ...
  vim.api.nvim_set_hl(0, 'OrangeBorder', { fg = '#FF9D65' })
  ...
  if vim.api.nvim_set_option_value then
    vim.api.nvim_set_option_value('winhl', 'FloatBorder:OrangeBorder', { scope = 'local', win = pwin })
  else
    vim.api.nvim_win_set_option(pwin, 'winhl', 'FloatBorder:OrangeBorder')
  end
  ...
  local function create_select_win()
    ...
    if vim.api.nvim_set_option_value then
      ...
      vim.api.nvim_set_option_value('winhl', '', { scope = 'local', win = swin })
    else
      vim.api.nvim_win_set_option(swin, 'cursorline', true)
      vim.api.nvim_win_set_option(swin, 'winhl', '')
    end
...
local function grep_items(on_list, query)
  ...
  for lnum, line in ipairs(lines) do
    local col = line:lower():find(query:lower(), 1, true)
    if col then
...
local function grep_item_text(item)
  local text = item.text:gsub('^%s*(.-)%s*$', '%1')
  return string.format('%s:%d:%d  %s', fileshorten(item.filename), item.lnum, item.col, text)
end

function M.pick_grep_current_file()
...
function M.setup()
  ...
  vim.api.nvim_create_autocmd('LspAttach', {
    ...
    callback = function(ev)
      ...
      vim.keymap.set('n', '<leader><leader>t', M.pick_definition, opts 'Picker: [D]efinition')
      vim.keymap.set('n', '<leader><leader>i', M.pick_implementation, opts 'Picker: [I]mplementation')
      vim.keymap.set('n', '<leader><leader>D', M.pick_type_definition, opts 'Goto type definition')
      vim.keymap.set('n', '<leader><leader>r', M.pick_references, opts 'Goto reference')
