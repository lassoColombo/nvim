...
local function extract_receiver(node)
  local query = vim.treesitter.query.parse(
    'go',
    [[
        (
...
return {
  function(node)
    ...
    local receiver = extract_receiver(node)
    ...
    local receiver_header = comment(indent(start_col + 2, '- receiver:'))
    table.insert(docstring, receiver_header)
    local receiver_arg = receiver:named_child(0)
    local fmtd_receiver = comment(indent(start_col + 4, helpers.node_text(receiver_arg) .. ': ...'))
    table.insert(docstring, fmtd_receiver)
